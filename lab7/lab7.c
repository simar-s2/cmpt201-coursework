...
void groupByKey(IntermediateInput *input, Output *output, int *result_count) {
  ...
  if (found_index != -1) {
    // Found existing entry - add line number to it
...
void reduce(Output *output) {
  ...
  // Print all line numbers with commas between them
